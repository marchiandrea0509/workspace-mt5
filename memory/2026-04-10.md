...
- MT5 journal workflow pivoted toward a local versioned Excel workbook as the primary artifact (`scripts/build_trade_journal_excel.py`, output under `reports/mt5_autotrade_phase1/excel/`). Policy for automation: always save a new dated workbook version, keep `MT5_trade_journal_latest.xlsx` updated, and only attempt Google Drive upload when a logged-in browser session is actually available; otherwise warn without failing the cycle.
